...
Windows 설치 패키지의 제품 버전은 `0.2.0`이며 NSIS 설치 프로그램 파일명에 같은 버전을 포함한다.
Windows Explorer 직접 실행 인자는 드라이브 문자가 있는 로컬 절대 `.md` 경로만 지원한다. 앱은 UNC 네트워크 경로와 드라이브 문자가 없는 경로에 접근하기 전에 이를 거부한다.
